...
def household_deconflict(household_links, individual_links, systems, config):
            ...
            if new[s]:
                found = next(
                    (
                        ...
                        if s in link.keys() and new[s] == link[s]
                    ),
                    None,
                )
                if found:
    ...
    print(
        "Exact individual links found in pprl household links: {}".format(
            str(metrics["equal"])
        )
    )
    print(
        "Number of individual links conflicting with pprl links: {}".format(
            str(metrics["conflict"])
        )
    )
    print(
        "Number of individual links combined into PPRL links: {}".format(
            str(metrics["combine"])
        )
    )
    print(
        "Number of individual links skipped from previous conflict: {}".format(
            str(metrics["skipped"])
        )
    )
...
def convert_ilinks_to_hlinks(individual_links, house_mappings, systems):
    ...
    for ilink in individual_links:
        ...
        for s in systems:
            if (
                s in ilink.keys()
                and house_mappings[s]
                and house_mappings[s][str(ilink[s])]
            ):
